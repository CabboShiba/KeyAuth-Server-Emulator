...
namespace KeyAuthServerEmulator.KeyAuthHelper.Cryptography.VarEmulator
{
    internal class GetVarEmulator
    {
        public static string Emulator(string Request)
        {
            ...
            foreach (var k in ok)
            {
                if (k.StartsWith("init_iv="))
                {
                    ...
                    encrypted = Cryptography.encrypt(PayLoad.GetVar, Credentials.EncKey, initiv);
                }

            }
            Logger.SaveLog($"KeyAuthGetVar: {PayLoad.GetVar}");
            return encrypted;
        }
    }
}

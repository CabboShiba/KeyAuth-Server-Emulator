...
{
    internal class UpgradeEmulator
    {
        public static string Emulator(string Request)
        {
            ...
            foreach (var k in SplitContent)
            {
                if (k.StartsWith("init_iv="))
                {
                    string[] init = k.Split('=');
                    InitIV = init[1];
                    EncryptedResponse = Cryptography.encrypt(PayLoad.Upgrade, Credentials.EncKey, InitIV);
                }

            }
            Logger.SaveLog($"KeyAuthRegister: {PayLoad.Upgrade}");
